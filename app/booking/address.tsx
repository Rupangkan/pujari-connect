/**
 * Address Selection — checkout step 1: select or add delivery address. Light theme.
 */
...
const SAVED_ADDRESSES = [
  { id: '1', fullName: 'Bishal Das', phone: '9999999999', flatHouse: '42, Divine Residency', area: 'Koramangala, 4th Block', city: 'Bangalore', state: 'Karnataka', pinCode: '560001', landmark: 'Near Forum Mall', isDefault: true },
];
        ...
        <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" contentContainerStyle={styles.content}>
          ...
          {showAddForm && (
            <Animated.View entering={FadeInDown.duration(250)}>
              ...
              <Pressable
                onPress={() => { if (isFormValid) setShowAddForm(false); }}
                disabled={!isFormValid}
                style={({ pressed }) => [styles.saveBtn, isFormValid && styles.saveBtnActive, pressed && { opacity: 0.85 }]}
              >
                <Text style={[styles.saveBtnText, isFormValid && styles.saveBtnTextActive]}>Save Address</Text>
              </Pressable>
            </Animated.View>
          )}
          ...
          <View style={[styles.bottomBar, { paddingBottom: insets.bottom + spacing.sm }]}>
            {selectedAddr && (
              <View style={styles.selectedRow}>
                ...
                <Text style={styles.selectedAddr} numberOfLines={1}>{selectedAddr.flatHouse}, {selectedAddr.city}</Text>
              </View>
            )}
            <Pressable onPress={() => router.push('/booking/payment')} style={({ pressed }) => [styles.continueBtn, pressed && { opacity: 0.85 }]}>
...
const styles = StyleSheet.create({
  ...
  selectedRow: { flexDirection: 'row', alignItems: 'center', gap: 4, marginBottom: spacing.sm },
  selectedAddr: { ...typography.bodySmall, color: colors.textMuted, flex: 1 },
